import { g as removeEventListener, h as raf, i as componentOnReady, t as addEventListener } from "./helpers-CcK7VbtN.js";
import { n as win, t as doc } from "./index9-Dbib3cpu.js";
import { n as KeyboardResize, t as Keyboard } from "./keyboard2-C5kYf6Hk.js";
import { i as findClosestIonContent, o as getScrollElement, u as scrollByPoint } from "./index8-DfmWCWxB.js";
//#region node_modules/@ionic/core/components/input-shims.js
/*!
...
var relocateInput = (componentEl, inputEl, shouldRelocate, inputRelativeY = 0, disabledClonedInput = false) => {
	if (cloneMap.has(componentEl) === shouldRelocate) return;
	if (shouldRelocate) addClone(componentEl, inputEl, inputRelativeY, disabledClonedInput);
	else removeClone(componentEl, inputEl);
};
var isFocused = (input) => {
	/**
...
var addClone = (componentEl, inputEl, inputRelativeY, disabledClonedInput = false) => {
	...
	* if we set opacity: 1.
	*/
	if (disabledClonedInput) clonedEl.disabled = true;
	const isRTL = componentEl.ownerDocument.dir === "rtl";
	if (isRTL) {
		const startOffset = parentEl.offsetWidth - inputEl.offsetLeft - inputEl.offsetWidth;
		clonedEl.style.insetInlineStart = `${startOffset}px`;
	} else clonedEl.style.insetInlineStart = `${inputEl.offsetLeft}px`;
...
var enableHideCaretOnScroll = (componentEl, inputEl, scrollEl) => {
	...
	const scrollHideCaret = (shouldHideCaret) => {
		if (isFocused(inputEl)) relocateInput(componentEl, inputEl, shouldHideCaret);
	};
...
var enableInputBlurring = () => {
	...
	const onTouchend = (ev) => {
		...
		if (!active) return;
		if (active.matches(SKIP_SELECTOR)) return;
		const tapped = ev.target;
		if (tapped === active) return;
		if (tapped.matches(SKIP_SELECTOR) || tapped.closest(SKIP_SELECTOR)) return;
		focused = false;
		setTimeout(() => {
...
var getScrollData = (componentEl, contentEl, keyboardHeight, platformHeight) => {
	var _a;
	return calcScrollData(((_a = componentEl.closest("ion-item,[ion-item]")) !== null && _a !== void 0 ? _a : componentEl).getBoundingClientRect(), contentEl.getBoundingClientRect(), keyboardHeight, platformHeight);
};
var calcScrollData = (inputRect, contentRect, keyboardHeight, platformHeight) => {
	...
	const distanceToBottom = visibleAreaBottom - SCROLL_AMOUNT_PADDING - inputBottom;
	const distanceToTop = safeAreaTop - inputTop;
	const desiredScrollAmount = Math.round(distanceToBottom < 0 ? -distanceToBottom : distanceToTop > 0 ? -distanceToTop : 0);
	const scrollAmount = Math.min(desiredScrollAmount, inputTop - visibleAreaTop);
	const duration = Math.abs(scrollAmount) / SCROLL_ASSIST_SPEED;
	return {
		scrollAmount,
		scrollDuration: Math.min(400, Math.max(150, duration)),
...
var setScrollPadding = (contentEl, paddingAmount, clearCallback) => {
	...
	if (paddingAmount > 0) contentEl.style.setProperty("--keyboard-offset", `${paddingAmount}px`);
	else contentEl[PADDING_TIMER_KEY] = setTimeout(() => {
...
var setClearScrollPaddingListener = (inputEl, contentEl, doneCallback) => {
	const clearScrollPadding = () => {
		if (contentEl) setScrollPadding(contentEl, 0, doneCallback);
	};
...
var enableScrollAssist = (componentEl, inputEl, contentEl, footerEl, keyboardHeight, enableScrollPadding, keyboardResize, disableClonedInput = false) => {
	...
	*/
	const focusOut = () => {
		...
		win === null || win === void 0 || win.removeEventListener("ionKeyboardDidShow", keyboardShow);
	...
	*/
	const focusIn = async () => {
		...
		*/
		...
		win === null || win === void 0 || win.addEventListener("ionKeyboardDidShow", keyboardShow);
...
var setManualFocus = (el) => {
	...
	*/
	...
	if (label !== null && label === activeElLabel) return;
	...
	*/
	...
	if (typeof window !== "undefined") {
		...
		const scrollContent = async () => {
			if (scrollContentTimeout !== void 0) clearTimeout(scrollContentTimeout);
			window.removeEventListener("ionKeyboardDidShow", doubleKeyboardEventListener);
			window.removeEventListener("ionKeyboardDidShow", scrollContent);
			if (contentEl) await scrollByPoint(contentEl, 0, scrollData.scrollAmount, scrollData.scrollDuration);
			...
			*/
			if (document.activeElement === inputEl) setManualFocus(inputEl);
			/**
			* When the input is about to be blurred
			* we should set a timeout to remove
			* any scroll padding.
			*/
			if (enableScrollPadding) setClearScrollPaddingListener(inputEl, contentEl, () => currentPadding = 0);
		};
			...
			*/
			...
			if (waitForResize && scrollData.scrollAmount > totalScrollAmount - scrollEl.scrollTop) {
				...
				*/
				...
				} else window.addEventListener("ionKeyboardDidShow", scrollContent);
				/**
...
var startInputShims = async (config, platform) => {
	...
	*/
	...
	const registerInput = async (componentEl) => {
		...
		if (!(inputEl.type === "date" || inputEl.type === "datetime-local") && (!!scrollEl || !!footerEl) && scrollAssist && !scrollAssistMap.has(componentEl)) {
	...
	if (inputBlurring && INPUT_BLURRING) enableInputBlurring();
	for (const input of inputs) registerInput(input);
...
//# sourceMappingURL=input-shims-Dp-LZX04.js.map