//#region node_modules/@stencil/core/internal/app-data/index.js
var BUILD = {
	allRenderFn: false,
	element: true,
	event: true,
...
//#endregion
//#region node_modules/@stencil/core/internal/client/index.js
...
var reWireGetterSetter = (instance, hostRef) => {
	...
	Object.entries((_a = cmpMeta.$members$) != null ? _a : {}).map(([memberName, [memberFlags]]) => {
...
var registerHost = (hostElement, cmpMeta) => {
	...
	if (!BUILD.lazyLoad && BUILD.modernPropertyDecls && (BUILD.state || BUILD.prop)) reWireGetterSetter(hostElement, hostRef);
	return ref;
};
...
var supportsListenerOptions = /* @__PURE__ */ (() => {
	...
	try {
		(_a = win.document) == null || _a.addEventListener("e", null, Object.defineProperty({}, "passive", { get() {
...
var consumeTimeout = (queue, timeout) => {
	let i2 = 0;
	let ts = 0;
	while (i2 < queue.length && (ts = performance.now()) < timeout) try {
		queue[i2++](ts);
	} catch (e) {
		consoleError(e);
	}
	if (i2 === queue.length) queue.length = 0;
	else if (i2 !== 0) queue.splice(0, i2);
};
var flush = () => {
	...
	static fromLocalValue(serialized) {
		...
		switch (type) {
			...
			case "number":
				...
				if (value === "-Infinity") return -Infinity;
				return value;
			case "array": return value.map((item) => _RemoteValue.fromLocalValue(item));
			...
			case "object":
				const obj = {};
				for (const [key, val] of value) obj[key] = _RemoteValue.fromLocalValue(val);
				return obj;
			case "regexp":
			...
			case "set":
				...
				for (const item of value) set.add(_RemoteValue.fromLocalValue(item));
				return set;
...
var updateFallbackSlotVisibility = (elm) => {
	...
	if (elm.tagName && elm.tagName.includes("-") && elm["s-cr"] && elm.tagName !== "SLOT-FB") getHostSlotNodes(childNodes, elm.tagName).forEach((slotNode) => {
		if (slotNode.nodeType === 1 && slotNode.tagName === "SLOT-FB") if (getSlotChildSiblings(slotNode, getSlotName(slotNode), false).length) slotNode.hidden = true;
		else slotNode.hidden = false;
	});
	let i2 = 0;
	for (i2 = 0; i2 < childNodes.length; i2++) {
		const childNode = childNodes[i2];
		if (childNode.nodeType === 1 && internalCall(childNode, "childNodes").length) updateFallbackSlotVisibility(childNode);
	}
};
...
function patchSlotNode(node) {
	...
	const assignedFactory = (elementsOnly) => (function(opts) {
		...
		if (opts == null ? void 0 : opts.flatten) console.error(`
		...
		if (elementsOnly) return toReturn.filter((n) => n.nodeType === 1);
		return toReturn;
	}).bind(node);
	...
	HostElementPrototype.cloneNode = function(deep) {
		...
		if (BUILD.slot && !isShadowDom && deep) {
			...
			for (; i2 < childNodes.length; i2++) {
				...
				if (nonStencilNode) clonedNode.appendChild(childNodes[i2].cloneNode(true));
			}
		}
		return clonedNode;
	};
...
var addStyle = (styleContainerNode, cmpMeta, mode) => {
	...
	if (!BUILD.attachStyles || !win.document) return scopeId2;
	...
	if (style) {
		if (typeof style === "string") {
			...
			if (!appliedStyles.has(scopeId2)) {
				if (BUILD.hydrateClientSide && styleContainerNode.host && (styleElm = styleContainerNode.querySelector(`[sty-id="${scopeId2}"]`))) styleElm.innerHTML = style;
				else {
		...
		} else if (BUILD.constructableCSS && !styleContainerNode.adoptedStyleSheets.includes(style)) if (supportsMutableAdoptedStyleSheets) styleContainerNode.adoptedStyleSheets.push(style);
		else styleContainerNode.adoptedStyleSheets = [...styleContainerNode.adoptedStyleSheets, style];
	}
	return scopeId2;
};
...
var hydrateScopedToShadow = () => {
	...
	let i2 = 0;
	for (; i2 < styles2.length; i2++) registerStyle(styles2[i2].getAttribute(HYDRATED_STYLE_ID), convertScopedToShadow(styles2[i2].innerHTML), true);
};
...
var initializeClientHydrate = (hostElm, tagName, hostId, hostRef) => {
	...
	for (; crIndex < crLength; crIndex++) {
		...
		if (!shadowRoot) {
			...
			if (cmpMeta) {
				...
				if (styleSheet) hostElm.shadowRoot.append(styleSheet.cloneNode(true));
			}
		}
	...
	for (; snIndex < snLen; snIndex++) {
		...
		for (; snGroupIdx < snGroupLen; snGroupIdx++) {
			...
			if (hostEle.shadowRoot && slottedItem.node.parentElement !== hostEle) hostEle.appendChild(slottedItem.node);
			if (!hostEle.shadowRoot || !shadowRoot) {
				...
				if (((_b = slottedItem.node.parentElement) == null ? void 0 : _b.shadowRoot) && slottedItem.node["getAttribute"] && slottedItem.node.getAttribute("slot")) slottedItem.node.removeAttribute("slot");
				if (BUILD.experimentalSlotFixes) patchSlottedNode(slottedItem.node);
			}
		}
	}
	if (BUILD.scoped && scopeId2 && slotNodes.length) slotNodes.forEach((slot) => {
		...
		if (rnLen) {
			...
			Array.from(hostElm.childNodes).forEach((node) => {
				if (typeof node["s-en"] !== "string" && typeof node["s-sn"] !== "string") {
					if (node.nodeType === 1 && node.slot && node.hidden) node.removeAttribute("hidden");
					else if (node.nodeType === 8 && !node.nodeValue || node.nodeType === 3 && !node.wholeText.trim()) node.parentNode.removeChild(node);
				}
			});
		}
	}
...
var clientHydrate = (parentVNode, childRenderNodes, slotNodes, shadowRootNodes, hostElm, node, hostId, slottedNodes = []) => {
	...
	if (node.nodeType === 1) {
			...
			if (childIdSplt[0] === hostId || childIdSplt[0] === "0") {
				...
				if (typeof slotName === "string") {
					if (childVNode.$tag$ === "slot-fb") {
						...
						if (BUILD.scoped && scopeId2) node.classList.add(scopeId2);
					}
		...
		for (i2 = nonShadowNodes.length - 1; i2 >= 0; i2--) clientHydrate(parentVNode, childRenderNodes, slotNodes, shadowRootNodes, hostElm, nonShadowNodes[i2], hostId, slottedNodes);
	...
	} else if (node.nodeType === 3 && !node.wholeText.trim() && !node["s-nr"]) node.remove();
	return parentVNode;
};
var initializeDocumentHydrate = (node, orgLocNodes) => {
	if (node.nodeType === 1) {
		...
		let i2 = 0;
		if (node.shadowRoot) for (; i2 < node.shadowRoot.childNodes.length; i2++) initializeDocumentHydrate(node.shadowRoot.childNodes[i2], orgLocNodes);
		const nonShadowNodes = node.__childNodes || node.childNodes;
		for (i2 = 0; i2 < nonShadowNodes.length; i2++) initializeDocumentHydrate(nonShadowNodes[i2], orgLocNodes);
	} else if (node.nodeType === 8) {
...
var selectorNeedsScoping = (selector, scopeSelector2) => {
	return !makeScopeMatcher(scopeSelector2).test(selector);
};
...
var applyStrictSelectorScope = (selector, scopeSelector2, hostSelector) => {
	scopeSelector2 = scopeSelector2.replace(/\[is=([^\]]*)\]/g, (_, ...parts) => parts[0]);
	const className = "." + scopeSelector2;
	const _scopeSelectorPart = (p) => {
		let scopedP = p.trim();
		if (!scopedP) return "";
		if (p.indexOf(_polyfillHostNoCombinator) > -1) scopedP = applySimpleSelectorScope(p, scopeSelector2, hostSelector);
		else {
...
var scopeCssText = (cssText, scopeId2, hostScopeId, slotScopeId, commentOriginalSelector) => {
	...
	if (scopeId2) cssText = scopeSelectors(cssText, scopeId2, hostScopeId, slotScopeId, commentOriginalSelector);
...
var parsePropertyValue = (propValue, propType, isFormAssociated) => {
	...
	if (propValue != null && !isComplexType(propValue)) {
		if (BUILD.propBoolean && propType & 4) if (BUILD.formAssociated && isFormAssociated && typeof propValue === "string") return propValue === "" || !!propValue;
...
var updateElement = (oldVnode, newVnode, isSvgMode2, isInitialRender) => {
	...
	if (BUILD.updatable) {
		for (const memberName of sortedAttrNames(Object.keys(oldVnodeAttrs))) if (!(memberName in newVnodeAttrs)) setAccessor(elm, memberName, oldVnodeAttrs[memberName], void 0, isSvgMode2, newVnode.$flags$, isInitialRender);
	}
	for (const memberName of sortedAttrNames(Object.keys(newVnodeAttrs))) setAccessor(elm, memberName, oldVnodeAttrs[memberName], newVnodeAttrs[memberName], isSvgMode2, newVnode.$flags$, isInitialRender);
};
...
var createElm = (oldParentVNode, newParentVNode, childIndex) => {
	...
	if (BUILD.slotRelocation && !useNativeShadowDom) {
		...
		if (newVNode2.$tag$ === "slot") newVNode2.$flags$ |= newVNode2.$children$ ? 2 : 1;
	}
	...
	if (BUILD.vdomText && newVNode2.$text$ !== null) elm = newVNode2.$elm$ = win.document.createTextNode(newVNode2.$text$);
		...
		if (BUILD.svg && isSvgMode && newVNode2.$tag$ === "foreignObject") isSvgMode = false;
		if (BUILD.vdomAttribute) updateElement(null, newVNode2, isSvgMode);
	...
	if (BUILD.slotRelocation) {
		if (newVNode2.$flags$ & 3) {
			...
			if (oldVNode && oldVNode.$tag$ === newVNode2.$tag$ && oldParentVNode.$elm$) if (BUILD.experimentalSlotFixes) relocateToHostRoot(oldParentVNode.$elm$);
			else putBackInOriginalLocation(oldParentVNode.$elm$, false);
...
var patch = (oldVNode, newVNode2, isInitialRender = false) => {
	...
	if (!BUILD.vdomText || text === null) {
		...
		if (BUILD.updatable && oldChildren !== null && newChildren !== null) updateChildren(elm, oldChildren, newVNode2, newChildren, isInitialRender);
		...
		else if (BUILD.hydrateClientSide && isInitialRender && BUILD.updatable && oldChildren !== null && newChildren === null) newVNode2.$children$ = oldChildren;
...
var markSlotContentForRelocation = (elm) => {
	...
	for (const childNode of children) {
		...
		if (childNode.nodeType === 1) markSlotContentForRelocation(childNode);
	}
};
...
var insertBefore = (parent, newNode, reference) => {
	if (BUILD.scoped && typeof newNode["s-sn"] === "string" && !!newNode["s-sr"] && !!newNode["s-cr"]) addRemoveSlotScopedClass(newNode["s-cr"], newNode, parent, newNode.parentElement);
		...
		if (parent.getRootNode().nodeType !== 11) patchParentNode(newNode);
		parent.insertBefore(newNode, reference);
		const { slotNode } = findSlotFromSlottedNode(newNode);
		if (slotNode) dispatchSlotChangeEvent(slotNode);
		return newNode;
	}
	if (BUILD.experimentalSlotFixes && parent.__insertBefore) return parent.__insertBefore(newNode, reference);
	else return parent == null ? void 0 : parent.insertBefore(newNode, reference);
};
function addRemoveSlotScopedClass(reference, slotNode, newParent, oldParent) {
	...
	if (reference && typeof slotNode["s-sn"] === "string" && !!slotNode["s-sr"] && reference.parentNode && reference.parentNode["s-sc"] && (scopeId2 = slotNode["s-si"] || reference.parentNode["s-sc"])) {
		...
		(_a = newParent.classList) == null || _a.add(scopeId2 + "-s");
  ...
  `);
				...
				if (slotRefNode) {
					...
					if (!BUILD.hydrateServerSide && (!BUILD.experimentalSlotFixes || insertBeforeNode && insertBeforeNode.nodeType === 1)) {
						...
						while (orgLocationNode) {
							...
							if (refNode && refNode["s-sn"] === nodeToRelocate["s-sn"] && parentNodeRef === (refNode.__parentNode || refNode.parentNode)) {
								...
								while (refNode === nodeToRelocate || (refNode == null ? void 0 : refNode["s-sr"])) refNode = refNode == null ? void 0 : refNode.nextSibling;
					...
					if (!insertBeforeNode && parentNodeRef !== parent || nextSibling !== insertBeforeNode) {
						if (nodeToRelocate !== insertBeforeNode) {
							if (!BUILD.experimentalSlotFixes && !nodeToRelocate["s-hn"] && nodeToRelocate["s-ol"]) nodeToRelocate["s-hn"] = nodeToRelocate["s-ol"].parentNode.nodeName;
							insertBefore(parentNodeRef, nodeToRelocate, insertBeforeNode);
							if (nodeToRelocate.nodeType === 1 && nodeToRelocate.tagName !== "SLOT-FB") nodeToRelocate.hidden = (_c = nodeToRelocate["s-ih"]) != null ? _c : false;
						}
					}
					nodeToRelocate && typeof slotRefNode["s-rf"] === "function" && slotRefNode["s-rf"](slotRefNode);
				} else if (nodeToRelocate.nodeType === 1) {
					if (isInitialLoad) nodeToRelocate["s-ih"] = (_d = nodeToRelocate.hidden) != null ? _d : false;
	...
	if (BUILD.experimentalScopedSlotChanges && cmpMeta.$flags$ & 2) {
		...
		for (const childNode of children) if (childNode["s-hn"] !== hostTagName && !childNode["s-sh"]) {
			if (isInitialLoad && childNode["s-ih"] == null) childNode["s-ih"] = (_e = childNode.hidden) != null ? _e : false;
...
var scheduleUpdate = (hostRef, isInitialLoad) => {
	if (BUILD.taskQueue && BUILD.updatable) hostRef.$flags$ |= 16;
...
var updateComponent = async (hostRef, instance, isInitialLoad) => {
	...
	if (BUILD.isDev) hostRef.$flags$ |= 1024;
	...
	if (BUILD.hydrateServerSide) try {
		...
		if (isInitialLoad) {
			if (hostRef.$cmpMeta$.$flags$ & 1) elm["s-en"] = "";
			else if (hostRef.$cmpMeta$.$flags$ & 2) elm["s-en"] = "c";
		}
	} catch (e) {
		consoleError(e, elm);
	}
...
var callRender = (hostRef, instance, elm, isInitialLoad) => {
	...
	try {
		...
		if (updatable && taskQueue) hostRef.$flags$ &= -17;
		...
		if (BUILD.hasRenderFn || BUILD.reflect) if (BUILD.vdomRender || BUILD.reflect) if (BUILD.hydrateServerSide) return Promise.resolve(instance).then((value) => renderVdom(hostRef, value, isInitialLoad));
			...
			if (hostRef.$cmpMeta$.$flags$ & 1) shadowRoot.textContent = instance;
...
var postUpdateComponent = (hostRef) => {
	...
	if (BUILD.isDev) hostRef.$flags$ &= -1025;
	...
	if (!(hostRef.$flags$ & 64)) {
		...
		if (BUILD.isDev) hostRef.$flags$ &= -2049;
...
var forceUpdate = (ref) => {
	...
	if (BUILD.updatable && (Build.isBrowser || Build.isTesting)) {
		...
		if (isConnected && (hostRef.$flags$ & 18) === 2) scheduleUpdate(hostRef, false);
		return isConnected;
	}
	return false;
};
var appDidLoad = (who) => {
	var _a;
	if (BUILD.asyncQueue) plt.$flags$ |= 2;
	nextTick(() => emitEvent(win, "appload", { detail: { namespace: "app" } }));
	if (BUILD.hydrateClientSide) {
		if ((_a = plt.$orgLocNodes$) == null ? void 0 : _a.size) plt.$orgLocNodes$.clear();
	}
...
var serverSideConnected = (elm) => {
	...
	if (children != null) for (let i2 = 0, ii = children.length; i2 < ii; i2++) {
	...
	if ((!BUILD.lazyLoad || !(flags & 8) || oldVal === void 0) && didValueChange) {
		...
		if (BUILD.serializer && BUILD.reflect && cmpMeta.$attrsToReflect$) {
			if (instance && cmpMeta.$serializers$ && cmpMeta.$serializers$[propName]) {
				let attrVal = newVal;
				for (const methodName of cmpMeta.$serializers$[propName]) attrVal = instance[methodName](attrVal, propName);
				hostRef.$serializerValues$.set(propName, attrVal);
			}
		}
		if (BUILD.isDev) {
			if (hostRef.$flags$ & 1024) consoleDevWarn(`The state/prop "${propName}" changed during rendering. This can potentially lead to infinite-loops and other bugs.`, "\nElement", elm, "\nNew value", newVal, "\nOld value", oldVal);
			else if (hostRef.$flags$ & 2048) consoleDevWarn(`The state/prop "${propName}" changed during "componentDidLoad()", this triggers extra re-renders, try to setup on "componentWillLoad()"`, "\nElement", elm, "\nNew value", newVal, "\nOld value", oldVal);
		}
...
var proxyComponent = (Cstr, cmpMeta, flags) => {
	...
	if (BUILD.formAssociated && cmpMeta.$flags$ & 64 && flags & 1) FORM_ASSOCIATED_CUSTOM_ELEMENT_CALLBACKS.forEach((cbName) => {
		...
		Object.defineProperty(prototype, cbName, { value(...args) {
			...
			if (!instance) (_a2 = hostRef == null ? void 0 : hostRef.$onReadyPromise$) == null || _a2.then((asyncInstance) => {
	...
	if (BUILD.member && cmpMeta.$members$ || BUILD.propChangeCallback) {
		...
		members.map(([memberName, [memberFlags]]) => {
			if ((BUILD.prop || BUILD.state) && (memberFlags & 31 || (!BUILD.lazyLoad || flags & 2) && memberFlags & 32)) {
				...
				if (flags & 1 || !origGetter) Object.defineProperty(prototype, memberName, {
					get() {
						if (BUILD.lazyLoad) {
							if ((cmpMeta.$members$[memberName][0] & 2048) === 0) return getValue(this, memberName);
				...
				Object.defineProperty(prototype, memberName, { set(newValue) {
					...
					if (!ref) return;
					if (BUILD.isDev) {
						if ((flags & 1) === 0 && (cmpMeta.$members$[memberName][0] & 4096) === 0 && (ref && ref.$flags$ & 8) === 0 && (memberFlags & 31) !== 0 && (memberFlags & 1024) === 0) consoleDevWarn(`@Prop() "${memberName}" on <${cmpMeta.$tagName$}> is immutable but was modified from within the component.
More information: https://stenciljs.com/docs/properties#prop-mutability`);
					}
					if (origSetter) {
						if (typeof (memberFlags & 32 ? this[memberName] : ref.$hostElement$[memberName]) === "undefined" && ref.$instanceValues$.get(memberName)) newValue = ref.$instanceValues$.get(memberName);
					...
					if (BUILD.lazyLoad) {
						if ((flags & 1) === 0 || (cmpMeta.$members$[memberName][0] & 4096) === 0) {
							setValue(this, memberName, newValue, cmpMeta);
							if (flags & 1 && !ref.$lazyInstance$) ref.$fetchedCbList$.push(() => {
								if (cmpMeta.$members$[memberName][0] & 4096 && ref.$lazyInstance$[memberName] !== ref.$instanceValues$.get(memberName)) ref.$lazyInstance$[memberName] = newValue;
							});
							return;
						}
						const setterSetVal = () => {
			...
			} else if (BUILD.lazyLoad && BUILD.method && flags & 1 && memberFlags & 64) Object.defineProperty(prototype, memberName, { value(...args) {
				var _a2;
				const ref = getHostRef(this);
		...
		if (BUILD.observeAttribute && (!BUILD.lazyLoad || flags & 1)) {
			...
			prototype.attributeChangedCallback = function(attrName, oldValue, newValue) {
				plt.jmp(() => {
					...
					if (BUILD.deserializer && cmpMeta.$deserializers$ && cmpMeta.$deserializers$[propName]) {
						...
						for (const methodName of cmpMeta.$deserializers$[propName]) if (BUILD.lazyLoad) if (hostRef.$lazyInstance$) setVal(methodName, hostRef.$lazyInstance$);
			...
			Cstr.observedAttributes = Array.from(/* @__PURE__ */ new Set([...Object.keys((_b = cmpMeta.$watchers$) != null ? _b : {}), ...members.filter(([_, m]) => m[0] & 31).map(([propName, m]) => {
				var _a2;
				const attrName = m[1] || propName;
				attrNameToPropName.set(attrName, propName);
				if (BUILD.reflect && m[0] & 512) (_a2 = cmpMeta.$attrsToReflect$) == null || _a2.push([propName, attrName]);
				return attrName;
			})]));
		}
	}
...
var initializeComponent = async (elm, hostRef, cmpMeta, hmrVersionId) => {
	...
	if ((hostRef.$flags$ & 32) === 0) {
		...
		if (BUILD.lazyLoad && bundleId) {
			...
			if (BUILD.member) hostRef.$flags$ &= -9;
			if (BUILD.propChangeCallback) hostRef.$flags$ |= 128;
			endNewInstance();
		...
		if (BUILD.style && Cstr && Cstr.style) {
			...
			if (!styles.has(scopeId2)) {
				...
				if (BUILD.hydrateServerSide && BUILD.shadowDom) {
					if (cmpMeta.$flags$ & 128) style = scopeCss(style, scopeId2, true);
		...
		if (!(hostRef.$flags$ & 1)) {
			...
			if (BUILD.hydrateClientSide) {
				...
				if (hostId) {
					...
					} else if (BUILD.scoped && cmpMeta.$flags$ & 2) elm["s-sc"] = getScopeId(cmpMeta, BUILD.mode ? elm.getAttribute("s-mode") : void 0);
					initializeClientHydrate(elm, cmpMeta.$tagName$, hostId, hostRef);
				}
			}
			if (BUILD.slotRelocation && !hostId) {
				if (BUILD.hydrateServerSide || (BUILD.slot || BUILD.shadowDom) && cmpMeta.$flags$ & 12) setContentReference(elm);
			}
			if (BUILD.asyncLoading) {
				let ancestorComponent = elm;
				while (ancestorComponent = ancestorComponent.parentNode || ancestorComponent.host) if (BUILD.hydrateClientSide && ancestorComponent.nodeType === 1 && ancestorComponent.hasAttribute("s-id") && ancestorComponent["s-p"] || ancestorComponent["s-p"]) {
			...
			if (hostRef == null ? void 0 : hostRef.$lazyInstance$) fireConnectedCallback(hostRef.$lazyInstance$, elm);
			else if (hostRef == null ? void 0 : hostRef.$onReadyPromise$) hostRef.$onReadyPromise$.then(() => fireConnectedCallback(hostRef.$lazyInstance$, elm));
		}
		endConnected();
	}
};
...
var disconnectedCallback = async (elm) => {
	if ((plt.$flags$ & 1) === 0) {
		...
		else if (hostRef == null ? void 0 : hostRef.$lazyInstance$) disconnectInstance(hostRef.$lazyInstance$, elm);
		else if (hostRef == null ? void 0 : hostRef.$onReadyPromise$) hostRef.$onReadyPromise$.then(() => disconnectInstance(hostRef.$lazyInstance$, elm));
	}
...
var proxyCustomElement = (Cstr, compactMeta) => {
	...
	if (!(cmpMeta.$flags$ & 1) && cmpMeta.$flags$ & 256) if (BUILD.experimentalSlotFixes) patchPseudoShadowDom(Cstr.prototype);
	else {
...
var addHostEventListeners = (elm, hostRef, listeners, attachParentListeners) => {
	if (BUILD.hostListener && listeners && win.document) {
		if (BUILD.hostListenerTargetParent) if (attachParentListeners) listeners = listeners.filter(([flags]) => flags & 32);
		else listeners = listeners.filter(([flags]) => !(flags & 32));
...
var hostListenerProxy = (hostRef, methodName) => (ev) => {
	var _a;
	try {
		if (BUILD.lazyLoad) if (hostRef.$flags$ & 256) (_a = hostRef.$lazyInstance$) == null || _a[methodName](ev);
...
//#endregion
export { createEvent as a, getMode as c, readTask as d, setMode as f, Host as i, h as l, Fragment as n, forceUpdate as o, writeTask as p, H as r, getAssetPath as s, Build as t, proxyCustomElement as u };

//# sourceMappingURL=client-C-sg6esh.js.map